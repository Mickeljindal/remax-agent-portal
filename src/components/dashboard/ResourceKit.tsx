...
import {
  Briefcase, FileText, ImageIcon, Mic, Presentation, FolderOpen, LinkIcon,
  Download, ExternalLink, Loader2, type LucideIcon,
} from "lucide-react";
...
interface ResourceKitProps {
  /** kit key in resource_kit_items, e.g. "listing" | "success" */
  kit: string;
  sectionKey: string;
  sectionId: string;
  defaultTitle: string;
...
export default function ResourceKit({
  kit, sectionKey, sectionId, defaultTitle, defaultSubtitle, HeaderIcon = Briefcase,
}: ResourceKitProps) {
  const [items, setItems] = useState<KitItem[]>([]);
  const [loading, setLoading] = useState(true);
  const { label } = useSectionLabels();
  const lbl = label(sectionKey, defaultTitle, defaultSubtitle);

  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data } = await supabase
        .from("resource_kit_items")
        .select("*")
        .eq("kit", kit)
        .eq("is_active", true)
        .order("sort_order");
  ...
  return (
    <section id={sectionId} className="scroll-mt-28 space-y-4">
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground py-6 text-center">No resources yet.</p>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item) => {
            const Icon = ICON_MAP[item.icon] || FileText;
            const hasFile = !!(item.file_url || item.link_url);
            const isLink = !item.file_url && !!item.link_url;
            return (
              <Card key={item.id} className="border-border/80 bg-card/90 shadow-sm transition hover:border-[#1a4d8f]/30">
                <CardContent className="flex flex-col gap-3 p-4">
                  ...
                  >
                    {hasFile ? (
                      isLink ? <><ExternalLink className="h-3.5 w-3.5" /> Open</> : <><Download className="h-3.5 w-3.5" /> Download</>
                    ) : "Coming soon"}
                  </Button>
                </CardContent>
              </Card>
